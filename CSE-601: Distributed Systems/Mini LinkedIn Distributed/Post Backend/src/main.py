...
@app.post('/register')
async def register_user(user_data: schemas.UserCreate, db: db):
    db_user = services.get_user(db, user_data.username)
    if db_user:
        raise HTTPException(status_code=400, detail="E-mail already Registered")
    access_token_expires = timedelta(minutes=services.ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = services.create_access_token(
...
@app.post('/token')
async def login_user(db: db, form_data: OAuth2PasswordRequestForm = Depends()):
    user_dict = services.get_user(db, form_data.username)
    if not user_dict:
        raise HTTPException(
            status_code=401,
            detail="Invalid username",
...
@app.get('/me')
async def get_current_user_info(db: db, token: str = Depends(oauth2_scheme)):
    token_data = services.verify_user(token)
    user = services.get_user(db, token_data.username)
    delattr(user, "password_hashed")
    return user


@app.post('/post')
async def create_post(db: db, post_text: str, image: UploadFile = File(None), token: str = Depends(oauth2_scheme)):
    token_data = services.verify_user(token)
    user = services.get_user_by_username(db, username=token_data.username)
    if user is None:
        raise services.credentials_exception
...
@app.get('/post', response_model=List[schemas.PostData])
async def get_posts(db: db, token: str = Depends(oauth2_scheme)):
    token_data = services.verify_user(token)
    user = services.get_user_by_username(db, username=token_data.username)
    if user is None:
        raise services.credentials_exception
...
@app.get('/notification')
async def get_notifications(db: db, token: str = Depends(oauth2_scheme)):
    token_data = services.verify_user(token)
    user = services.get_user_by_username(db, username=token_data.username)
    if user is None:
        raise services.credentials_exception
